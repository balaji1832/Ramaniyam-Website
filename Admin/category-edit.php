...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['category_name'])) {
    	...
    	$statement = $pdo->prepare("SELECT * FROM tbl_our_category WHERE id=?");
		$statement->execute(array($_REQUEST['id']));
		$result = $statement->fetchAll(PDO::FETCH_ASSOC);
		foreach($result as $row) {
			$current_country_name = $row['category_name'];
		}

		$statement = $pdo->prepare("SELECT * FROM tbl_our_category WHERE category_name=? and category_name!=?");
    	$statement->execute(array($_POST['category_name'],$current_country_name));
    ...
    if($valid == 1) {    	
		...
		$statement = $pdo->prepare("UPDATE tbl_our_category SET category_name=? WHERE id=?");
