...
<section class="content-header">
	<div class="content-header-left">
		<h1>View Brochure Enquiry  Details</h1>
	</div>
</section>
...
<section class="content">
	<div class="row">
	  ...
	  <?php if($filter!="") {
               ...
               $msg="Selected user $_GET[action] Successfully";  
    ...
    <a href="?filter=archived"><button class="<?php if($_GET['filter']=='archived') { echo 'active'; } ?>">Archived Enquiries</button></a>
			...
			<div class="box box-info">
				...
				// 										FROM enquiry_new 
				// 										where type='Brochure' and is_archived=0
				// 									");
				// 			$statement->execute();
                            ...
                            switch ($filter) {
                            case 'archived':
                            $stmt = $pdo->prepare("SELECT * FROM enquiry_new WHERE  type='Brochure' and is_archived = 1 ORDER BY id DESC ");
                            break;
                            case 'active':
                            $stmt = $pdo->prepare("SELECT * FROM enquiry_new WHERE   type='Brochure' and is_archived = 0 ORDER BY id DESC");
                            break;
                            case 'msg':
                            $stmt = $pdo->prepare("SELECT * FROM enquiry_new WHERE   type='Brochure' and is_archived = 0 ORDER BY id DESC");
                            break;
                            default:
                            $stmt = $pdo->prepare("SELECT * FROM enquiry_new WHERE   type='Brochure' ORDER BY id DESC");
                            break;
                            }
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure want to Archive this item?</p>
            </div>
...
<script>
    function changeStatus(id, action) {
        if (confirm("Are you sure you want to " + action + " this user?")) {
            window.location.href = `customer.php?action=${action}&id=${id}`;
        }
    }
</script>
