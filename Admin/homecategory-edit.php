...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
		...
		if($path == '') {
			$statement = $pdo->prepare("UPDATE tbl_home_banner_bottom SET heading=?, content=?, button_text=?, button_url=?, position=? WHERE id=?");
			...
			$final_name = 'homecategory-'.$_REQUEST['id'].'.'.$ext;
        	move_uploaded_file( $path_tmp, '../assets/uploads/home/'.$final_name );

        	$statement = $pdo->prepare("UPDATE tbl_home_banner_bottom SET photo=?, heading=?, content=?, button_text=?, button_url=?, position=? WHERE id=?");
...
if(!isset($_REQUEST['id'])) {
	...
	$statement = $pdo->prepare("SELECT * FROM tbl_home_banner_bottom WHERE id=?");
...
<section class="content-header">
	...
	<div class="content-header-right">
		<a href="homecategory.php" class="btn btn-primary btn-sm">View All</a>
	</div>
</section>
