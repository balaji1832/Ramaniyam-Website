...
if(isset($_POST['form1'])) {
    ...
    $path_new = $_FILES['secondary_featured_photo']['name'];
    $path_tmp_new = $_FILES['secondary_featured_photo']['tmp_name'];

    if($path_new!='') {
        $ext_new = pathinfo( $path_new, PATHINFO_EXTENSION );
        $file_name_new = basename( $path_new, '.' . $ext_new );
        if( $ext_new!='jpg' && $ext_new!='png' && $ext_new!='ext_new' && $ext_new!='gif' ) {
            $valid = 0;
            $error_message .= 'You must have to upload jpg, jpeg, gif or png file<br>';
        }
    } 
    ...
    if($valid == 1) {
    	...
    	if( isset($_FILES['photo']["name"]) && isset($_FILES['photo']["tmp_name"]) )
        {
        	...
        	$statement = $pdo->prepare("SELECT pp_id FROM tbl_product_photo ORDER BY pp_id DESC  Limit 1");
			$statement->execute();
        ...
        $p_qty = $_POST['p_qty']; $p_description = $_POST['p_description']; $p_short_description = $_POST['p_short_description'];
        $p_features = $_POST['p_feature']; $newarrivals = $_POST['newarrivals']; $bestsellers = $_POST['bestsellers'];
        $p_condition = $_POST['p_return_policy']; $p_is_featured = $_POST['p_is_featured']; $p_is_active = $_POST['p_is_active'];
        $tcat_id = $_POST['tcat_id']; $mcat_id = $_POST['mcat_id']; $ecat_id = $_POST['ecat_id']; $pId=$_REQUEST['id'];
        

        
         $query = "UPDATE tbl_product SET p_name=:fullname, p_old_price=:oldprice, p_current_price=:currentprice,p_qty=:qty
        ,p_description=:description,p_short_description=:shortdescription,p_condition=:pconditions,p_is_featured=:featured,p_feature=:dfeatures, p_is_newarrivals=:newarrivals , 
        p_is_bestsellers=:bestsellers,p_return_policy=:returnpolicy,p_is_active=:productactivestatus,tcat_id=:topcategory,mcat_id=:midlevelcategory,
        ecat_id=:endlevelcategory
        ...
        $data = [
            ...
            ':pconditions' => $p_condition,
            ':dfeatures' => $p_features,
            ':featured' => $p_is_featured,
            ':newarrivals' => $newarrivals,
            ':bestsellers' => $bestsellers,
            ':returnpolicy' => $p_condition,
            ':productactivestatus' => $p_is_active,
            ':topcategory' => $tcat_id,
            ':midlevelcategory' => $mcat_id,
            ':endlevelcategory' => $ecat_id,
            ':p_id' => $pId
        ];
        ...
        if($query_execute)
        {
        	...
        	unlink('../assets/uploads/'.$_POST['current_secondryphoto']);

			$file_name_new = 'product-secondary-featured-'.$_REQUEST['id'].'.'.$ext_new;
        	move_uploaded_file( $path_tmp_new, '../assets/uploads/'.$file_name_new);
...
foreach ($result as $row) {
	...
	$secondary_featured_photo = $row['p_secondry_photo'];

	$p_condition = $row['p_condition'];
	$p_return_policy = $row['p_return_policy'];
	$p_is_featured = $row['p_is_featured'];
	$p_is_newarrivals = $row['p_is_newarrivals'];
	$p_is_bestsellers= $row['p_is_bestsellers'];
...
<section class="content-header">
	...
	<div class="content-header-right">
		<a href="product-edit.php?<?php echo $prev_link;?>" class="btn btn-primary btn-sm"> Previous Product </a> &nbsp;	<a href="product-edit.php?<?php echo $next_link;?>" class="btn btn-primary btn-sm"> Next Product </a>
	</div>
</section>
			...
			<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
				...
				<div class="box box-info">
					<div class="box-body">
						...
						<!--<div class="form-group">-->
						<!--	<label for="" class="col-sm-3 control-label">Existing Secondary Featured Photo</label>-->
						<!--	<div class="col-sm-4" style="padding-top:4px;">-->
						<!--		<img src="../assets/uploads/<?php echo $secondary_featured_photo; ?>" alt="" style="width:150px;">-->
						<!--		<input type="hidden" name="current_secondryphoto" value="<?php echo $secondary_featured_photo; ?>">-->
						<!--	</div>-->
						<!--</div>-->
						<!--<div class="form-group">-->
						<!--	<label for="" class="col-sm-3 control-label">Change Secondary Featured Photo </label>-->
						<!--	<div class="col-sm-4" style="padding-top:4px;">-->
						<!--		<input type="file" name="secondary_featured_photo">-->
						<!--	</div>-->
						<!--</div>-->
						<div class="form-group">
			                        	...
			                        	foreach ($result as $row) {
											...
											<tr>
				                                <td>
				                                 ...
				                                 <td style="width:28px;">
				                                	<a href="product-photo-edit.php?id=<?php echo $row['pp_id']; ?>&id1=<?php echo $_REQUEST['id']; ?>" class="btn btn-info btn-xs">Edit</a>
				                                </td>&nbsp;
				                                <td style="width:28px;">
						...
						<div class="form-group">
							...
							<div class="form-group">
							<label for="" class="col-sm-3 control-label">Is New Arrivals?</label>
							<div class="col-sm-8">
								<select name="newarrivals" class="form-control" style="width:auto;">
									<option value="0" <?php if($p_is_newarrivals == '0'){echo 'selected';} ?>>No</option>
									<option value="1" <?php if($p_is_newarrivals == '1'){echo 'selected';} ?>>Yes</option>
								</select> 
							</div>
						</div>
						
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Is Bestsellers?</label>
							<div class="col-sm-8">
								<select name="bestsellers" class="form-control" style="width:auto;">
									<option value="0" <?php if($p_is_bestsellers == '0'){echo 'selected';} ?>>No</option>
									<option value="1" <?php if($p_is_bestsellers == '1'){echo 'selected';} ?>>Yes</option>
								</select> 
							</div>
						</div>
