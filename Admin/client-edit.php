...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
		...
		if($path == '') {
			$statement = $pdo->prepare("UPDATE tbl_client SET title=?, content=? WHERE id=?");
    		$statement->execute(array($_POST['title'],$_POST['content'],$_REQUEST['id']));
		} else {

			unlink('../assets/uploads/'.$_POST['current_photo']);

			$final_name = 'client-'.$_REQUEST['id'].'.'.$ext;
        	move_uploaded_file( $path_tmp, '../assets/uploads/'.$final_name );

        	$statement = $pdo->prepare("UPDATE tbl_client SET  photo=? WHERE id=?");
...
<section class="content-header">
	...
	<div class="content-header-right">
		<a href="client.php" class="btn btn-primary btn-sm">View All</a>
	</div>
</section>
					...
					<div class="box-body">
						...
						<div class="form-group">
							...
							<div class="col-sm-6" style="padding-top:5px">
								<input type="file" required name="photo">(Only jpg, jpeg, gif and png are allowed)
							</div>
						</div>	
						<div class="form-group">
