...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['category_name'])) {
        $valid = 0;
        $error_message .= "Home Category Filter can not be empty<br>";
    } else {
    	// Duplicate Category checking
    	$statement = $pdo->prepare("SELECT * FROM tbl_our_category WHERE category_name=?");
    ...
    if($valid == 1) {
		...
		$statement = $pdo->prepare("INSERT INTO tbl_our_category (category_name) VALUES (?)");
		$statement->execute(array($_POST['category_name']));
	
    	$success_message = 'Home Category Filter  is added successfully.';
    }
}
?>
