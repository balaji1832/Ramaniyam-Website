...
if(isset($_POST['form1'])) {
    ...
    if(isset($_FILES['photo_logo']) && $_FILES['photo_logo']['error'] == 0) {
        ...
        if($path == '') {
            ...
            if(!in_array(strtolower($ext), $allowed_exts)) {
                $valid = 0;
                $error_message .= 'You must upload a JPG, JPEG, PNG, GIF, or SVG file.<br>';
            }
        }
    } else {
        $valid = 0;
    ...
    if($valid == 1) {
        ...
        if($result) {
            ...
            // Remove the existing file only if it exists and is not empty
        ...
        if(move_uploaded_file($path_tmp, $destination)) {
            ...
            $error_message .= 'Failed to move uploaded file.<br>';
        }
    }
...
if(isset($_POST['form2'])) {
    ...
    if(isset($_FILES['photo_favicon']) && $_FILES['photo_favicon']['error'] == 0) {
        $path = $_FILES['photo_favicon']['name'];
        $path_tmp = $_FILES['photo_favicon']['tmp_name'];
    ...
    if($valid == 1) {
        ...
        if($result) {
            ...
            // Remove existing favicon only if it exists
            if(!empty($favicon) && file_exists($favicon_path)) {
                unlink($favicon_path);
            }
        }
...
if(isset($_POST['form3'])) {
    ...
    // updating the database
    $statement = $pdo->prepare("UPDATE tbl_settings SET footer_copyright=?, contact_address=?, contact_email=?, contact_phone=?, contact_fax=? WHERE id=1");
    $statement->execute(array($_POST['footer_copyright'],$_POST['contact_address'],$_POST['contact_email'],$_POST['contact_phone'],$_POST['contact_fax']));
                        ...
                        <div class="tab-pane" id="tab_3">
                            ...
                            <div class="box box-info">
                                <div class="box-body">
                                 ...
                                 <div class="form-group">
                                        <label for="" class="col-sm-2 control-label">Contact Secondary Phone Number </label>
                                        <div class="col-sm-6">
